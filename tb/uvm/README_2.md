...
* **`tb/uvm/`**: A small **UVM verification environment** for the core, with
  one file per feature:
  * **Constrained-random stimulus** (`aes_stimulus.svh`) — random plaintexts
    and keys, weighted so the edge cases (all 0s, all 1s, `AAAA…`, `5555…`)
    are actually hit.
  * **Golden-model scoreboard** (`aes_scoreboard.svh`, `aes_reference.svh`) —
    compares the hardware against an independent SystemVerilog reference,
    validated on NIST FIPS-197 and SP 800-38A vectors.
  * **SVA** (`aes_assertions.sv`) — proves the `valid_in` → `valid_out`
    timing contract.
  * **Functional coverage** (`aes_coverage.svh`) — reports which edge cases
    were exercised.

  Run it with `cd tb/uvm && vsim -c -do run.do`. See
  [`tb/uvm/README.md`](tb/uvm/README.md).
  * *Note:* building this environment surfaced a latency deviation — the
    spec calls for 10 cycles, the current FSM takes 12. The ciphertext is
    correct on every vector. Details in `tb/uvm/README.md`, section "Latency".
* **`tb_aes_top.v`**: The original directed testbench. It acts as the "environment" that:
